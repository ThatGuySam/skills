...
Use Measure when quotes are missing, the scope is unfamiliar, evidence is sparse, or more research may not change the decision. The scenarios below connect each problem to a method and a prompt you can adapt.
...
- The U.S. Bureau of Labor Statistics' [Producer Price Index concepts](https://www.bls.gov/opub/hom/ppi/concepts.htm) distinguish transaction prices from list prices. Transaction prices account for the buyer, terms, discounts, premiums, rebates, and allowances.
